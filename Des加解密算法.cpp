...
void roundkey(string pt,string key,vector<string> *rkb,vector<string> *rk)
{
    //以下为子密钥生成过程
    //将密钥从16进制转换为2进制
    key = hex2bin(key);
...
int main()
{
 ...
 //pt为明文，key为密钥
    ...
    vector<string> rkb; //2进制密钥容器
    vector<string> rk; //16进制密钥容器

    //为16轮密钥生成函数
    roundkey(pt,key, &rkb, &rk);
